...
const getOrderByUserId = async (req, res) => {
  ...
  const getOrderByUserOrderAndProductId = async (req, res) => {
    try {
      ...
      if (productItem.returnDetails.isReturned || productItem.cancelDetails.isCancel) {
        return res.status(404).json({ message: "This product is either returned or canceled" });
      }
